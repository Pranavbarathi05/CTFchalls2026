...
rebuilder ="Fapmk eqqye xuwq m oxagpk pmk. U'xx euf mf tayq mzp imfot etadfe."
rebuild() {
  local arr=("$@") out=""
  for n in "${arr[@]}"; do
...
┌──(ashish㉿ASHISH)-[/mnt/c/Users/ANIL KUMAR]
└─$ nano tester.txt
...
rebuilder ="Xaaw uzfa ftq bmef itqdq ymzk yustfk eqqwqde raxxaiqp ftq fdmux fa huofadk"
rebuild() {
  local arr=("$@") out=""
  for n in "${arr[@]}"; do
    out+=$(printf "\\x%02x" "$(( (n - 17) ^ K ))")
